"""
Financial Statement Ingest and Regeneration View

재무제표 수집과 재생성을 한번에 처리하는 통합 View입니다.
"""
...
from financial_statements.tasks import ingest_and_regenerate_financial_statements
...
class FinancialStatementIngestAndRegenerationForm(forms.Form):
  """재무제표 수집 및 재생성 폼"""
...
class FinancialStatementIngestAndRegenerationView(UnfoldModelAdminViewMixin, FormView):
  """재무제표 수집 및 재생성 통합 커스텀 페이지

  Raw 데이터 수집 후 자동으로 정제된 데이터를 생성합니다.
  """

  title = _("Queue financial statement ingest and regeneration")
  permission_required = ()
  template_name = "admin/companies/company/financial_statement_ingest_and_regeneration_form.html"
  form_class = FinancialStatementIngestAndRegenerationForm
  ...
  def get_context_data(self, **kwargs):
    ...
    context["action"] = self.request.POST.get("action") or "queue_financial_statement_ingest_and_regeneration"
    return context

  def form_valid(self, form: FinancialStatementIngestAndRegenerationForm) -> HttpResponse:
    """폼 검증 성공 시 수집 및 재생성 실행"""
    ...
    try:
      ...
      if async_results:
        ...
        messages.success(
          self.request,
          _(
            "Queued financial statement ingest and regeneration for %(count)d companies (%(summary)s)."
      ...
      logger.error(f"재무제표 수집 및 재생성 실패: {e}")
      messages.error(self.request, f"수집 및 재생성 실패: {str(e)}")
