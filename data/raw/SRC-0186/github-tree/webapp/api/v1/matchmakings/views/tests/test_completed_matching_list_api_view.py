...
class CompletedMatchingListAPIViewTest(APITestCase):
  """CompletedMatchingListAPIView 테스트"""
  ...
  def setUp(self):
    ...
    self.url = reverse("matching-completed-list")

  def test_get_completed_matchings_success_as_sender(self):
    """보낸 호감이 수락되어 완료된 매칭 목록 조회 성공 테스트"""
    # 완료된 매칭 생성 (수락됨)
  ...
  def test_get_completed_matchings_success_as_receiver(self):
    """받은 호감을 수락하여 완료된 매칭 목록 조회 성공 테스트"""
    # 완료된 매칭 생성 (수락됨)
    matching = MatchingFactory(
  ...
  def test_get_completed_matchings_multiple(self):
    """여러 개의 완료된 매칭 목록 조회 테스트"""
  ...
  def test_get_completed_matchings_excludes_pending(self):
    """대기 중인 매칭은 제외되는지 테스트"""
    # 완료된 매칭
    completed_matching = MatchingFactory(
        sender=self.user,
        receiver=self.other_user,
        sent_at=timezone.now(),
        received_at=timezone.now(),
    ...
    self.assertEqual(response.status_code, status.HTTP_200_OK)
    self.assertEqual(response.data["count"], 1)
    self.assertEqual(response.data["results"][0]["id"], completed_matching.id)

  def test_get_completed_matchings_excludes_rejected(self):
    """거절된 매칭은 제외되는지 테스트"""
    # 완료된 매칭
    completed_matching = MatchingFactory(
        sender=self.user,
  ...
  def test_get_completed_matchings_empty(self):
    """완료된 매칭이 없을 때 빈 목록 반환 테스트"""
    self.client.force_authenticate(user=self.user)
    response = self.client.get(self.url)
  ...
  def test_get_completed_matchings_unauthenticated(self):
    """인증되지 않은 사용자의 요청은 401 에러 테스트"""
    response = self.client.get(self.url)

    self.assertIn(response.status_code, [status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN])

  def test_get_completed_matchings_pagination(self):
    """페이지네이션 동작 테스트"""
    # 여러 개의 완료된 매칭 생성
    for i in range(15):
    ...
    # StandardPagination 기본 page_size가 10이라고 가정
    self.assertLessEqual(len(response.data["results"]), 15)

  def test_get_completed_matchings_only_user_related(self):
    """현재 사용자와 관련된 매칭만 조회되는지 테스트"""
