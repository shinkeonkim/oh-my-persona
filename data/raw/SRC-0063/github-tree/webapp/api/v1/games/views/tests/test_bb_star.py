...
class BBStarStartAPIViewTests(TestCase):
    """BBStarStartAPIView 테스트"""
    ...
    def setUp(self):
        ...
        self.url = "/api/v1/games/bb-star/start/"
    ...
    def test_start_bb_star_game_success(self):
        """뿅뿅 아기별 게임 시작 성공 테스트"""
        ...
        self.assertEqual(response.data["game_code"], GameCodeChoice.BB_STAR)
    ...
    def test_start_bb_star_game_without_authentication(self):
        """인증 없이 게임 시작 실패 테스트"""
        response = self.client.post(self.url, {}, format="json")

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_start_bb_star_game_without_child(self):
    ...
    def test_start_bb_star_game_with_inactive_game(self):
    ...
    def test_start_bb_star_game_multiple_sessions(self):
...
@pytest.mark.django_db
class BBStarFinishAPIViewTests(TestCase):
    """BBStarFinishAPIView 테스트"""
    ...
    def setUp(self):
        ...
        self.url = "/api/v1/games/bb-star/finish/"
    ...
    def test_finish_bb_star_game_success(self):
        """뿅뿅 아기별 게임 종료 성공 테스트"""
        self.client.force_authenticate(user=self.user)
        ...
        self.assertEqual(response.data["wrong_count"], 2)
        self.assertEqual(response.data["round_count"], 8)
        self.assertEqual(response.data["success_count"], 6)

        # DB에 게임 결과가 생성되었는지 확인
        result = GameResult.objects.get(session=self.session)
        self.assertEqual(result.score, 85)
        self.assertEqual(result.wrong_count, 2)
        self.assertEqual(result.round_count, 8)
    ...
    def test_finish_bb_star_game_minimal_data(self):
        """최소 데이터로 게임 종료 테스트"""
        self.client.force_authenticate(user=self.user)
    ...
    def test_finish_bb_star_game_without_authentication(self):
        """인증 없이 게임 종료 실패 테스트"""
        data = {
            "session_id": str(self.session.id),
            "score": 85,
    ...
    def test_finish_bb_star_game_invalid_session_id(self):
    ...
    def test_finish_bb_star_game_already_completed(self):
    ...
    def test_finish_bb_star_game_other_user_session(self):
    ...
    def test_finish_bb_star_game_missing_score(self):
        """점수 없이 게임 종료 실패 테스트"""
        self.client.force_authenticate(user=self.user)
    ...
    def test_finish_bb_star_game_no_reaction_time(self):
        """뿅뿅 아기별은 반응시간이 없어야 함"""
        self.client.force_authenticate(user=self.user)
        ...
        data = {
            "session_id": str(self.session.id),
            "score": 85,
            "reaction_ms_sum": 5000,  # 반응시간 포함
        }

        response = self.client.post(self.url, data, format="json")

        # 성공하지만 reaction_ms_sum은 serializer에서 제거되므로 저장되지 않음
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        result = GameResult.objects.get(session=self.session)
        self.assertIsNone(result.reaction_ms_sum)  # BB_STAR는 reaction_ms_sum을 저장하지 않음
