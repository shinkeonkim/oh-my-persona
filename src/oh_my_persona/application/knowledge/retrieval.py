    ...
    def __init__(self, root: Path):
        ...
        if processed:
            ...
            for path in iter_corpus_files(root):
                for generated_chunk in chunk_text(
                    path.read_text(encoding="utf-8"), str(path.relative_to(root))
                ):
                    hit = SearchHit(
                        generated_chunk.chunk_id,
                        generated_chunk.text,
                        0.0,
                        metadata={"source_path": generated_chunk.source_path},
                    )
                    self.records.append((hit, Counter(tokenize(generated_chunk.text))))
        ...
        for hit, tokens in self.records:
            ...
            if score:
                ...
                if hit.metadata.get("claim_id"):
                    ...
                    score *= 1.45
                scored.append(replace(hit, score=round(score, 6)))
...
class PostgresHybridRetriever:
    def __init__(self, database_url: str, embedding: Any):
