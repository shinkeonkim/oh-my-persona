...
Routes live in the presentation package; this module owns process-wide dependencies.
"""
...
from ..infrastructure.llm.strands_agent import ALLOWED_MODELS, stream_invoke
from .admin_router import create_admin_router
from .http_support import enforce_rate_limit, sse
...
def public_managed_knowledge(item_id: str) -> dict[str, object]:
...
def create_widget_session_api(request: Request) -> dict[str, str]:
...
def widget_chat(
    request: WidgetChatRequest, http_request: Request, background: BackgroundTasks
) -> dict[str, object]:
