...
import type { AbuseStatus, ConversationMessage, ConversationSummary } from "../../api/types";
import { AbuseControls } from "./AbuseControls";
...
export function ConversationsPanel({ token }: { token: string }) {
  ...
  const [abuse, setAbuse] = useState<AbuseStatus>({ blocked: false });
  useEffect(() => { void load(); }, []);
  ...
  async function open(id: string) { const data = await requestJson<{ messages: ConversationMessage[]; abuse?: AbuseStatus }>(`/api/admin/conversations/${id}`, {}, token); setSelected(id); setMessages(data.messages); setAbuse(data.abuse ?? { blocked: false }); }
